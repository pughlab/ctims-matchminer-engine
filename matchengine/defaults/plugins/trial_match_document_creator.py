...
class CustomTrialMatchDocumentCreator(TrialMatchDocumentCreator):
    ...
    def _format_genomic_inclusion_match(self, match_reason: MatchReason, genomic_doc: dict):
        ...
        if variant_category == 'MUTATION':
            hugo_symbol = genomic_doc.get('HARMONIZED_HUGO_SYMBOL')
            protein_change = genomic_doc.get('HARMONIZED_PROTEIN_CHANGE')
            if protein_change:
            ...
            variant_classification = genomic_doc.get("HARMONIZED_VARIANT_CLASSIFICATION")
    ...
    def _get_sort_order(self, match_document: Dict) -> list:
        ...
        protocol_digits = [c for c in str(match_document.get("protocol_no") or "") if c.isdigit()]
        if protocol_digits:
            pid = int("".join(protocol_digits))
            sort_array.append(pid)
        else:
            sort_array.append(99)
    ...
    _SIGNATURE_MAP = {
        ...
        'tobacco_signature': ('TOBACCO_STATUS', 'Tobacco Signature'),
    }
