...
def find_plugins(matchengine: MatchEngine):
    ...
    for dir_path, module_name in to_load:
        ...
        for item_name in getattr(module, '__shared__', list()):
            log.debug(f"Found shared plugin resource {item_name} in module {module_name}, path {dir_path}")
        ...
        for item_name in module.__export__:
            ...
            if issubclass(item, QueryTransformerContainer):
                log.debug(f"Loading QueryTransformerContainer: {item_name}")
                ...
                if item_name == matchengine.match_document_creator_class:
                    log.debug(f"Loading TrialMatchDocumentCreator: {item_name}")
                    instance = item(matchengine)
                    setattr(matchengine,
                            'create_trial_matches',
                            instance.create_trial_matches)
            elif issubclass(item, DBSecrets):
                if item_name == matchengine.db_secrets_class:
                    log.debug(f"Loading DBSecrets: {item_name}")
                ...
                if item_name == matchengine.query_node_transformer_class:
                    log.debug(f"Loading QueryNodeTransformer: {item_name}")
                    setattr(matchengine,
                ...
                if item_name == matchengine.query_node_subsetter_class:
                    log.debug(f"Loading QueryNodeClinicalIDsSubsetter: {item_name}")
                    setattr(matchengine,
                ...
                if item_name == matchengine.query_node_container_transformer_class:
                    log.debug(f"Loading QueryNodeContainerTransformer: {item_name}")
                    setattr(matchengine,
